...
if [ -f "$JOURNALCTL" ]; then
    ...
    # 4) 부팅 경계 추출: "-- Boot" 마커 + 전후 5줄
    #    systemd 저널이 찍는 부팅 구분선. 재부팅 시점 목록 + 직전/직후 문맥 제공
    #    - Boot 마커 직전 = 이전 세션 마지막 이벤트 (종료 원인 단서)
    ...
    # 각 Boot 경계 전후 5줄씩 추출 (부팅 간 전환 문맥)
    if [ "$BOOT_COUNT" -gt 0 ]; then
        grep -n -B5 -A5 '^\-\- Boot' "$JOURNALCTL" \
...
5. **전처리 파일 Read** (`_preprocessed/` 디렉터리):
   ...
   - `journalctl-boot-context.txt` — 각 Boot 경계 전후 5줄 (종료 원인·복구 문맥)
