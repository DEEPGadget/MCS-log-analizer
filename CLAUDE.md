...
10. **시간 상관관계 분석**: `/home/deepgadget/MCS-log-analyzer/correlation-guide.md`를 Read하여
    방법론을 확인하고 이벤트 묶음을 구성한다
...
2. **보고서 구조 검증**: 보고서가 모든 필수 섹션을 포함하는지 확인한다:
   ...
   - 분석 제외 항목
   - 환경 특이사항
3. **보고서 형식**: `/home/deepgadget/MCS-log-analyzer/report-template.md`를 Read하여
   형식을 확인한 후, `Write` 도구로 지정된 경로에 저장한다
...
#### `_preprocessed/journalctl-errors.txt` / `_preprocessed/journalctl-lifecycle.txt`

Pass 1 step 5에서 이미 Read한 전처리 파일을 기반으로 분석한다. 원본 `journalctl.txt`는 Grep하지 않는다.

**journalctl-errors.txt** (에러/경고 필터링 결과):
- `Failed to start` / `failed with result` → **Critical/Warning**: 서비스 시작 실패
- `BUG:` / `Oops:` / `kernel NULL pointer` → **Critical**: 커널 버그/크래시
...
마크다운 보고서의 섹션 구조와 표 형식은 `/home/deepgadget/MCS-log-analyzer/report-template.md`를 참조한다.
시간 상관관계 분석 방법론은 `/home/deepgadget/MCS-log-analyzer/correlation-guide.md`를 참조한다.
